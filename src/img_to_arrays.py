...
"""
Turn png files (original and the stepwise reconstructions) into h5py objects
with numpy array representations of the images.
Also create a h5py object with the pretrained image embeddings.

The constants MODEL_NAME and SHAPE have to be manually set accordingly!
...
SHAPE = 2048
RESIZE_AND_CENTER = False
CLIPARTS = Path('../data/CoDraw-master/Pngs/')
CODRAW_PATH = Path('../data/CoDraw-master/dataset/CoDraw_1_0.json')
# sequential reconstructions by the drawer
IMAGES = Path('../data/CoDrawImages/DrawerScenes/')
# original image seen by the teller
ORIG_IMAGES = Path('../data/AbstractScenes_v1.1/RenderedScenes/')
SPLITS = ('train', 'val', 'test')
DEVICE = 'cuda'

OUTPUT_DIR = Path(f'../data/preprocessed/images/{MODEL_NAME}/')
os.mkdir(OUTPUT_DIR)
OUTPUT_RAW = Path('../data/preprocessed/images/raw/')
try:
    os.mkdir(OUTPUT_RAW)
...
# Following PS's code and https://discuss.pytorch.org/t/how-can-l-use-the-pre-trained-resnet-to-extract-feautres-from-my-own-dataset/9008/6
...
with torch.no_grad():
    for split in SPLITS:
        ...
        with h5py.File(output_images, 'w') as f1:
            with h5py.File(output_embeddings, 'w') as f2:
                for game_id, n_rounds in tqdm(idxs[split].items()):
                    ...
                    f2.create_dataset(game_id, data=sequence_embeddings)
        ...
        with h5py.File(output_images, 'w') as f1:
            with h5py.File(output_embeddings, 'w') as f2:
                for game_id, _ in tqdm(idxs[split].items()):
                    ...
                    f1.create_dataset(game_id, data=img)
                    f2.create_dataset(game_id, data=output)
    assert counter == 10
